import { pokemonDetailSevices, pokemonListSevices } from "@/services";
...
const DetailPage = () => {
  ...
  const callData = async (name: string) => {
    const response = await pokemonDetailSevices.getPokemonDetail(name);
  ...
  return (
    <div className="w-[90 %] m-[auto] max-w-[1100px]">
      ...
      <div className="w-[90%] max-w-[600px] m-[auto]">
        <Link
          to={"/"}
          className="bg-[#4DB5F1] px-[16px] py-[4px] rounded-[16px] font-semibold"
        >
          Back
        </Link>
        {pokemon.data && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-[20px] gap-y-[30px]">
                <div>
                  <div className="flex gap-x-[10px]">
                    <div className="text-[#4DB5F1] font-semibold">Height</div>
                    <div className="text-white">
                      {(pokemon.data.height / 10).toFixed(2)} m.
                    </div>
                  </div>
                  <div className="flex gap-x-[10px]">
                    <div className="text-[#4DB5F1] font-semibold">Weight</div>
                    <div className="text-white">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-1 gap-[10px] mt-[16px]">
                    {pokemon.data.abilities.map((item) => {
                      return (
                        <span
                          className={`bg-[#4DB5F1] px-[14px] capitalize py-1 rounded-[16px]`}
                        >
                          {item.ability.name}
                        </span>
                      );
                    ...
                    {pokemon.data.stats.map((item) => {
                      return (
                        <div className="flex gap-x-[10px] justify-between">
                          <div className="text-[#4DB5F1] font-semibold capitalize">
                            {item.stat.name}
                          </div>
