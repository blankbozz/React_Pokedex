...
import { pokemonListSevices, pokemonDetailSevices } from "@/services";
...
const useSearchForm = () => {
  ...
  const callData = async (filter: {
    ...
    const responseList = await pokemonListSevices.getPokemonList(
      filter.limit,
      filter.offset
    );
    const pokeList = [];
    ...
    if (responseList.status === 200) {
      ...
      for (const pokemon of responseResults) {
        const response = await pokemonDetailSevices.getPokemonDetail(
          pokemon.name
        );
